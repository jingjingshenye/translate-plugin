...
import type { Storage as StorageType } from 'webextension-polyfill'
...
function guessSerializerType(rawInit: unknown) {
  return rawInit == null
    ...
    : Number.isNaN(rawInit) ? 'any'
    : 'number'
}

const storageInterface: StorageLikeAsync = {
  removeItem(key: string) { return storage.local.remove(key) },
  setItem(key: string, value: string) { return storage.local.set({ [key]: value }) },
  async getItem(key: string) { const data = await storage.local.get(key); return data[key] as string },
}
...
export function useWebExtensionStorage<T>(
    ...
    try {
      ...
      if (rawValue == null) {
        ...
        if (typeof mergeDefaults === 'function')
          ...
          data.value = { ...(rawInit as Record<string, unknown>), ...(value as Record<string, unknown>) } as T
  ...
  const { pause: pauseWatch, resume: resumeWatch } = pausableWatch(data, write, { flush, deep, eventFilter })

  if (listenToStorageChanges) {
    const listener = async (changes: Record<string, StorageType.StorageChange>) => {
      try {
        pauseWatch()
    ...
    tryOnScopeDispose(() => storage.onChanged.removeListener(listener))
  }

  return { data: data as RemovableRef<T>, dataReady: dataReadyPromise }
}
